...
/* Returns radius multiplier for current mode */
function getRadiusMult() {
...
out vec4 fragColor;
void main(){
  vec3 paper = vec3(0.941, 0.922, 0.878);
  vec4 d;
  if (uCalliMode) {
...
window.addEventListener('mousemove', e => {
  ...
  if (!isDown && currentMode !== 'calli') {
    ...
    if (spd > 0.5) {
      splatDye(mx / W, 1.0 - my / H, INKS[inkIdx], 0.0003 * 0.4, 0.006);
    }
  }
});
...
window.addEventListener('mouseleave', () => { isDown = false; lastSplatX = -1; lastSplatY = -1; });
...
window.addEventListener('touchend',  () => { isDown = false; lastSplatX = -1; lastSplatY = -1; });
...
function step() {
  ...
  gl.uniform1i(pDisplay.u('uVelocity'), 1);
